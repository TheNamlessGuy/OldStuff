...
public class Pawn {
	...
	public static JButton[][] canMove(int[] location, JButton[][] b, Color[][] colors, ImageIcon[] enemies, JButton q, ImageIcon QG) {
		...
		if (location[0] == 0){
			b = Pawn.toQueen(b, location, QG);
		}
		b = Pawn.canAttack(b, enemies, location);
		if (canMoveB){
	...
	public static JButton[][] toQueen(JButton[][] b, int[] location, ImageIcon QG){
		b[location[0]][location[1]].setIcon(QG);
		return b;
	}
